...
import { saveSnapshotOPFS, loadSnapshotOPFS, mirrorToLocalStorage, requestPersistentStorage } from './utils/snapshot';
...
React.useEffect(() => {
  (async () => {
    try {
      // Ask for persistent storage to reduce eviction risk (Chromium)
      await requestPersistentStorage();

      const hasLocal = (Array.isArray(videos) && videos.length > 0) || (Array.isArray(playlists) && playlists.length > 0);
      if (hasLocal) return;
